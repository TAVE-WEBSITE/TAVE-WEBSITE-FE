...
export default function HomeTest() {
  ...
  const [innerWidth, setInnerWidth] = useState(
    window.visualViewport.width <= 758
  );
  let totalPage = innerWidth ? 4 : 5;
  window.addEventListener("resize", () => {
    setInnerWidth(window.visualViewport.width <= 758);
  ...
  useEffect(() => {
    ...
    const handleScroll = (deltaY) => {
      ...
      smoothScrollTo(nextPage * pageHeight);
...
<button
        ...
        >
          지금 바로 지원하기
            ...
            {/* 웹에서는 이 문구가 따로 */}
            <div className="hidden md:flex h-svh w-full flex-col items-center justify-center w-screen md:px-20 px-5 ">
                <div className="text-center md:title-1 text-xl font-semibold leading-6 tracking-[-0.9px] z-10 relative">
                    다양한 분야와의 융합을 통해 미래를 선도하는
                    <br /> IT 연합 동아리, TAVE를 소개합니다
                </div>

                <div
                    className="absolute top-full w-full h-full bg-cover bg-no-repeat"/>
                </div>
  ...
  return (
    ...
    <div className="outer relative mx-auto break-keep" ref={outerDivRef}>
      ...
      {/*두번째 화면 */}
      <div
        className="h-svh w-full flex flex-col items-center justify-center max-md:pt-14  md:px-20 px-5 bg-no-repeat md:bg-none"
        style={{
          backgroundImage: backgroundImage,
          backgroundPositionY: bgPosition2,
      ...
      >
        ...
        <div className="flex flex-col items-center md:gap-4">
          ...
          <div className="md:text-xl font-light max-md:text-[16px]">
            TAVE의 핵심가치를 소개합니다
          </div>
        </div>
        <div className="hidden md:scale-75 lg:scale-95  md:flex gap-4 mt-12 px-4 md:gap-12 md:mt-[50px] md:px-[148px]">
      ...
      {/*세번째 화면 */}
      <div
        className="h-svh w-full flex flex-col justify-center items-center md:px-20 px-5 bg-cover bg-center bg-no-repeat"
        style={{
      ...
      >
        {" "}
        <div className="md:text-4xl font-bold title-1 max-md:text-[26px] md:mb-4">
          TAVE HISTORY
        </div>
        <div className="md:text-xl font-light max-md:text-[16px] leading-[20px] md:mb-[50px] mb-12 text-center">
      ...
      {/*다섯번째 화면 */}
      <div
        className="realtive h-dvh flex flex-col w-full"
        style={{
