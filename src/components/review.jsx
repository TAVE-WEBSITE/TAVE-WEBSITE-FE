...
export default function Review() {
    ...
    return (
        <div className="flex gap-x-20 w-full">
            <button type="button" onClick={prevSlide} disabled={canNext} className="disabled:cursor-not-allowed">
                <img src={ReviewIcon} />
            </button>
            <div className="flex flex-col gap-y-12 w-full">
                {paddedReview.slice(currentIndex, currentIndex + 2).map(
                    (review, index) =>
                        review && (
                            <div
                                className={`w-full flex-grow flex ${index % 2 === 0 ? 'justify-start' : 'justify-end'}`}
                                key={index}
                            >
                                <div
            ...
            <button type="button" onClick={nextSlide} disabled={canPrev} className="disabled:cursor-not-allowed">
